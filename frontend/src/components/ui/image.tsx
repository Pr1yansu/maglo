...
export const Image = forwardRef<HTMLImageElement, ImageProps>(
    (
        ...
        const [placeholderStyle, setPlaceholderStyle] = useState<React.CSSProperties | undefined>(undefined);
        ...
        useEffect(() => {
            ...
            const pick = breakpoints.reduce((acc, b) => (Math.abs(b - (width || 768)) < Math.abs(acc - (width || 768)) ? b : acc), breakpoints[0]);
        ...
        useEffect(() => {
            ...
            const node = (finalRef as React.MutableRefObject<HTMLImageElement | null>).current;
        ...
        const imgStyle: React.CSSProperties = {
            width: layout === "fill" || layout === "responsive" ? "100%" : width ? `${width}px` : undefined,
            height: layout === "fill" || layout === "responsive" ? "100%" : height ? `${height}px` : undefined,
            objectFit: "cover",
            display: "block",
            ...style,
        };
        ...
        const picture = (
            <picture>
                {formats.map((fmt) => (
                    <source
                        key={fmt}
                        type={fmt === "jpg" || fmt === "jpeg" ? "image/jpeg" : `image/${fmt}`}
                        srcSet={
                            isVisible
                                ? breakpoints
                                    .map((w) => `${buildUrl(fmt, w)} ${w}w`)
                                    .join(", ")
                                : undefined
                        }
                        sizes={sizes}
                ...
                <img
                    ...
                    sizes={sizes}
                    alt={alt}
                    loading={useNativeLazyLoad ? imgProps.loading || "lazy" : imgProps.loading || (priority ? "eager" : "lazy")}
                    style={imgStyle}
                    className={className}
                    width={width}
                    height={height}
        ...
        return layout === "responsive" || layout === "intrinsic" || layout === "fill" ? (
            <div
                className={cn(
                    "relative",
            ...
            <>
                {placeholder && !isVisible ? (
                    <span className="inline-block" style={{ ...placeholderStyle, width: width || undefined, height: height || undefined }} />
                ) : null}
                {picture}
            </>
        );
